...
Install the required project-local Universal Notifier checkout before running the backend:

```bash
./scripts/install_project_dependencies.sh
```

This clones or updates `https://github.com/mundosoftware/Universal-Notifier.git` at `.deps/Universal-Notifier` and prompts for the `mundosoftware` GitHub password or token. Set `UNIVERSAL_NOTIFIER_REPO_URL`, `UNIVERSAL_NOTIFIER_REPO_USERNAME`, or `UNIVERSAL_NOTIFIER_REPO_PATH` to override the source, username, or checkout location. When `UNIVERSAL_NOTIFIER_ENABLED=true`, deployment syncs this source checkout but continues to exclude its private `.env.local`.
...
When `UNIVERSAL_NOTIFIER_ENABLED=true`, deployment also syncs the project-local `.deps/Universal-Notifier` source, installs Node.js 20/npm, and manages a separate `${SERVICE_NAME}-notifier` systemd service. The notifier's private `.env.local` is never synced or generated by this project; it must already exist at `${APP_DIR}/.deps/Universal-Notifier/.env.local` on the VPS. When the flag is false, deployment stops and disables that service. The app-facing `UNIVERSAL_NOTIFIER_URL` and `UNIVERSAL_NOTIFIER_HTTP_TOKEN` values are written to the API runtime environment without exposing notifier provider credentials in this repository.

Trade Alert exposes `/readyz` with the notifier state, and nginx proxies `/v1/events` through the public VPS origin, so Postman can use the same host as `API_BASE` for `UNIVERSAL_NOTIFIER_BASE`. The Node service remains bound to VPS loopback and port 8787 is not exposed directly.
