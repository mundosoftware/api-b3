...
actor CompanionAPIClient {
    static let shared = CompanionAPIClient()
    ...
    func registerIOSDevice(userId: String, subscriptionId: String) async throws {
        ...
        let _: DeviceRegistrationResponse = try await send(
            "/users/\(userId)/devices/ios",
            method: "POST",
            body: body
        )
    }
    ...
    func createAIOutlookJob(
        userId: String,
        request: AIOutlookJobCreateRequest
    ) async throws -> AIOutlookJob {
        try await send("/users/\(userId)/ai-outlook/jobs", method: "POST", body: request)
    }

    func aiOutlookJob(userId: String, jobId: String) async throws -> AIOutlookJob {
    ...
    private func sendNoContent(_ path: String, method: String) async throws {
        ...
        guard (200..<300).contains(http.statusCode) else {
            throw CompanionAPIError.badResponse(http.statusCode, "")
        }
    }
    ...
    private func decode<T: Decodable>(_ request: URLRequest) async throws -> T {
        ...
        guard let http = response as? HTTPURLResponse else {
            throw CompanionAPIError.badResponse(-1, "")
        }
        guard (200..<300).contains(http.statusCode) else {
            let message = String(data: data, encoding: .utf8) ?? ""
            throw CompanionAPIError.badResponse(http.statusCode, message)
        }
